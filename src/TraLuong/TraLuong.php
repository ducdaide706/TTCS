...
// Đếm tổng số bản ghi trong bảng traluong
$sql_count = "SELECT COUNT(*) as total FROM traluong t JOIN nhanvien n ON t.MaNV = n.MaNV WHERE n.HoTen LIKE ?";
...
if (!$result_count) {
    die("Lỗi truy vấn đếm tổng số bản ghi: " . $conn->error);
}
$total_records = $result_count->fetch_assoc()['total'];
$totalPages = ceil($total_records / $records_per_page);

// Truy vấn danh sách trả lương
$sql = "SELECT t.MaTraLuong, n.HoTen, n.NgaySinh, n.GioiTinh, p.TenPhongBan, c.TenCV, t.Thang, t.Nam, t.TongLuong
        FROM traluong t
                ...
                <div class="page-title-box d-flex align-items-center justify-content-between">
                    ...
                    <div class="page-title-right">
                        <ol class="breadcrumb m-0">
                            ...
                            <li class="breadcrumb-item active">Quản Lý Trả Lương</li>
                        </ol>
                    </div>
                </div>
            </div>
                                        ...
                                        <tbody>
                                            <?php foreach ($list as $key => $value): ?>
                                                <tr role="row" class="odd">
                                                    ...
                                                    <td> <?php echo $value['Thang']; ?>/<?php echo $value['Nam']; ?></td>
                                                    <td><?php echo number_format($value['TongLuong']); ?> VNĐ</td>
                                                    <td>
                                                        <a class="btn btn-secondary" style="width: 100%;" href="/QLNhanVien_thuan/src/TraLuong/SuaTraLuong.php?id=<?php echo $value['MaTraLuong']; ?>">XEM</a>
                                                    </td>
                                                </tr>
                                            <?php endforeach ?>
                                        </tbody>
                                    ...
                                    <ul class="pagination pagination-rounded">
                                        <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                                            <li style="margin-right: 5px;" class="paginate_button page-item <?php echo $i === $page ? 'active' : ''; ?>">
                                                <a href="/QLNhanVien_thuan/src/TraLuong/TraLuong.php?path=tra-luong/trang&trang=<?php echo $i; ?><?php echo $search_query ? '&search=' . urlencode($search_query) : ''; ?>" class="page-link"><?php echo $i; ?></a>
                                            </li>
                                        <?php endfor; ?>
                                    </ul>
                                </div>
